...
struct fio_option fio_options[FIO_MAX_OPTS] = {
	...
	{
		.name	= "max_parallel_zones",
		.lname	= "Maximum number of parallel read or write zones",
		.type	= FIO_OPT_INT,
		.off1	= offsetof(struct thread_options, max_parallel_zones),
		.maxval	= ZBD_MAX_OPEN_ZONES,
		.help	= "Maximum number of parallel zones for read or write",
		.def	= "0",
		.category = FIO_OPT_C_IO,
		.group	= FIO_OPT_G_INVALID,
	},
